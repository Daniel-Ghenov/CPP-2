...
double stringToDouble(char* array, int dotPlace){
    double wholePart = 0;
    double fraction = 0;
...
bool checkString(char* array, int& dotPos){
...
void checkAllStrings(){
    ...
    for(size_t i {0}; i < numberOfStrings; i++){
        int dotPos = 0;
        if(checkString(allStrings[i], dotPos)){
            allDoubles[doubleSize++] = stringToDouble(allStrings[i], dotPos);
        }
    }

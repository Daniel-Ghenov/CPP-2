...
void printAllAnagrams(char* largerString, char* compareString, int* compareStringCount){
	...
	for(size_t i {0}; i < largerSize; i++){
        if(isAnagram(largerString, largerSize, compareString, compareSize, compareStringCount, i))
			std::cout<<", "<<i;
        else if(isAnagram(largerString, largerSize, compareString, compareSize, compareStringCount, i) && !hasAnagrams){
			std::cout<<i;
			hasAnagrams = true;
		}
	}

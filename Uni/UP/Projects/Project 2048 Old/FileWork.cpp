...
char** getLeaderboard(int fieldSize, char* filename, int* scoreList){
    ...
    for(size_t i {0};!inputFile.eof() && i < LEADERBOARD_LENGTH; i++){
        inputFile.getline(nicknameList[i], LEADERBOARD_LENGTH, ' ');
...
void printLeaderboard(){
    ...
    for(size_t i {0}; i < LEADERBOARD_LENGTH; i++){
        if(nicknameList[i][0] != ' ' && nicknameList[i][0] != '\0')
            std::cout<<i + 1<<":  "<<scoreList[i]<<"  Nickname: "<<nicknameList[i];
        else 
            break;
    }
    std::cout<<std::endl;
